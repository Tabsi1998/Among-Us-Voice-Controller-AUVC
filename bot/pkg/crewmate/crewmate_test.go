...
func TestAnAnnouncedDeathShowsTheGhost(t *testing.T) {
	board := Render([]session.GamePlayer{
...
func TestTheBoardWorksBeforeThePicturesAreUploaded(t *testing.T) {
	board := Render([]session.GamePlayer{{Name: "Alice", Color: game.Red, Alive: true}}, nil, nil)
...
func TestTheMenuStaysWithinDiscordsLimit(t *testing.T) {
	...
	options := menuOf(t, Render(players, nil, nil)).Options
...
func TestANameCannotFormatTheBoard(t *testing.T) {
	board := Render([]session.GamePlayer{{Name: "**@everyone**", Color: game.Red, Alive: true}}, nil, nil)
